...
config = {
    ...
    'Environment Name' : 'SuperMarioBros-v0',                           # Env given to each worker. Play with 'MsPacman-v0' or 'SuperMarioBros-v0' as well.
    'Number of worker threads' : 8,                                     # Number of parallel envs on their own thread.

    # Sample loop variables
    'Number of environment episodes' : 1024,                            # How many times we reboot test envs.
    'Max Number of sample batches per environment episode' : 64,        # May end earlier.
    'Max steps taken per batch' : 2048,                                 # Number steps agent takes in env.
    'Max timsteps' : 1024 * 64,                                         # Episodes * batches. likely will end far before as envs may terminate early.
    
    # Training loop variables
    'Training epochs' : 10,                                             # Number of times we train on a sample gathered from env.
    'Mini batches per training epoch' : 32,                             # How many updates per epoch per batch.
    ...
    # CNN options
    'CNN type' : 'large',                                               # 'large' vs. 'small.' Means one has more convolution layers.

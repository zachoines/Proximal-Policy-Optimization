...
class Train():
    ...
    def start(self):
        ...
        if self._config['CNN type'] == 'large':
            ...
            self._global_model = AC_Model_Small(self.NUM_STATE, self.NUM_ACTIONS, self._config, is_training=True)
            self._global_model(tf.convert_to_tensor(np.random.random((1, *self.NUM_STATE)), dtype='float64'))
            self._step_model = AC_Model_Small(self.NUM_STATE, self.NUM_ACTIONS, self._config, is_training=True)
            self._step_model(tf.convert_to_tensor(np.random.random((1, *self.NUM_STATE)), dtype='float64'))
